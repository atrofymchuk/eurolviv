...
export const Header: React.FC<RoomHeaderToProps> = ({ header }) => {
  ...
  return (
    <div className="w-full relative">
      ...
      <Link
        to="/rooms"
        className="absolute top-[12%] left-[3%] text-[#C7C7C7] uppercase text-[16px] leading-[15.12px] px-[10px] flex gap-2 items-center hover:cursor-pointer z-20"
      >
       <BsArrowLeft />    номери
      </Link>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white z-10">
        ...
        <h1 className="text-white uppercase whitespace-pre-line font-cofo xl:text-[198px] lg:text-[100px] md:text-[80px] flex flex-col text-[48px] font-normal z-20 xl:leading-[154.83px] leading-[39.13px] tracking-[-0.1em] text-center underline decoration-transparent">
          <span>{titlePart[0]}</span>
          <br />
          <span>{titlePart[1] || ""}</span>
        </h1>

        <p
          className={`uppercase lg:leading-[35px] lg:text-[42px] text-[20px]  font-semibold leading-[25px] xl:pt-[46px] md:text-[32px] lg:pt-[30px] pt-[8px]`}
        >
          {t(size || "")}
        </p>

        <p
          className={`font-cofo uppercase lg:text-[18px] lg:leading-[22px] lg:w-[531px] w-[291px] ${
            size ? "lg:pt-[46px]" : ""
          } pt-[20px] text-[12px] leading-[15.12px] px-[10px]`}
        >
          {t(description)}
        </p>
      </div>
