    ...
    <div className="w-full flex justify-center">
      <div className="grid lg:grid-cols-2 grid-cols-1 w-full">
        {suggestion.map((item, index) => (
          <div key={index} className="bg-[#252526] relative pt-[240px] flex flex-col">
            <div className="relative">
              <div className="absolute inset-0 z-10 bg-gradient-to-b from-[#252526] to-transparent"></div>
              <img src={item.img} alt={item.type} className="lg:w-full lg:h-[654px] object-cover" />
            </div>

            <div className="absolute top-0 flex flex-col items-center text-center text-white z-10 p-4 w-full h-[250px] justify-s">
            <h1 className="lg:text-[100px] text-[32px] lg:leading-[81px] uppercase leading-[28px] mt-20 min-h-[162px] flex items-end">
  {item.title}
</h1>

              { item.size.length > 5 && <p className="uppercase font-semibold lg:text-[24px] text-[14px] lg:leading-[24px] leading-[12px] lg:pt-[19px] pt-[7px]">
                {item.size}
              </p>}
