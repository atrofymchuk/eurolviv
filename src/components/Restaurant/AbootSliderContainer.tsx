...
export const AbootSliderContainer: React.FC<AbootSliderContainerProps> = ({
  ...
  useEffect(() => {
    ...
    const preventScrollToSlider = (e: TouchEvent) => {
    ...
    if (currentContainer && isMobile) {
      currentContainer.addEventListener("touchmove", preventScrollToSlider, {
        passive: true,
      });
    }
  ...
  return (
    <div
      ...
      className={`relative flex xl:h-[50vw] items-center lg:justify-end   max-w-screen   lg:h-[650px] md:h-[570px] flex-col 
        ${
          isTerrase
            ? "w-full h-[380px] 2xl:h-[46.15vw] "
            : "justify-center h-[370px] 2xl:h-[46.15vw]"
        }`}
    >
      <div className="absolute w-full h-[660px] -z-1 flex items-end justify-center ">
        <div
          className={cn(`absolute h-[660px] xl:h-[50vw] 2xl:h-[46.15vw]  `, {
            "2xl:w-[93.1%] xl:w-[93.1%] lg:w-[93.1%] md:w-[93.1%] w-[89.07%]":
              !isTerrase,
        ...
        >
          <div className="h-full grid grid-cols-1  w-full md:grid-cols-[49.9%_0.1%_50.03%]">
            ...
            <div
              className={` border-s  ${
                isTerrase ? " md:border-s" : " md:border-s-0"
              } ${
                index === 2 ? "h-2/3" : ""
              }  border-[#B3B3B3] hidden md:block`}
            ></div>
      ...
      <div
        className={`flex flex-col items-center lg:mb-0 ${
          isTerrase ? "mt-12" : "mt-1 "
        }`}
      >
        {/* На мобільних обгортаємо в додатковий div для правильної обробки touch-подій */}
        ...
        <button
          className={cn(
            `2xl:text-[.73vw] lg:text-[14px] 2xl:mt-[38px]  font-cofo-medium text-center
